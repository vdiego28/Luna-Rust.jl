...
@testitem "Native-Rust CUDA radial EnvGrid SDO Raman" tags=[:rust] begin
    ...
    if !isfile(libpath)
        require_cuda && error("CUDA radial EnvGrid Raman tests require the Rust library")
        ...
        function make_radial(; rotation=false, vibration=true, energy=3e-5)
            grid = Grid.EnvGrid(flength, λ0, (150e-9, 2000e-9), 0.5e-12)
            ...
            raman = Nonlinear.RamanPolarEnv(grid.to, rr)
            responses = (Nonlinear.Kerr_env(PhysData.γ3_gas(gas)), raman)
        ...
        # Construction is the hardware gate. Strict mode must fail rather
        # than silently accepting a CPU backend when CUDA is unavailable.
        gpu_error = try
        ...
        if gpu_error !== nothing
            require_cuda && error("CUDA radial EnvGrid Raman setup failed: $gpu_error")
            @test_skip "CUDA GPU/toolkit not available: $(sprint(showerror, gpu_error))"
        else
            @testset "Direct stages: vibration and N2 rotation" begin
                for (name, c) in (("vibration", vib), ("rotation", rot))
                    ...
                    println("CUDA radial EnvGrid Raman $name stage rel: $rel")
                    @test rel < 1e-10
                end
            end
            ...
            @testset "Fixed solve and Julia non-vacuity" begin
                ...
                println("CUDA radial EnvGrid Raman fixed-solve rel: $rel")
                @test rel < 1e-6
                ...
                println("Julia radial EnvGrid Raman on/off rel: $effect")
                @test effect > 1e-6
            end

            @testset "Complex column isolation" begin
                c = vib
                s_cpu = make_stepper(c; gpu=false)
                s_gpu = make_stepper(c; gpu=true)
                ...
                @test set_radial(s_gpu._handle.ptr, c, T_identity, 1.0, 1.0,
                                 window, 0.0, M) == 0
                @test set_raman(s_cpu._handle.ptr, c) == 0
                @test set_raman(s_gpu._handle.ptr, c) == 0
                isolated = zeros(ComplexF64, c.n)
                isolated[1] = 2e14 + 1e14im
                isolated[c.n_spec + 1] = 1e14 - 0.5e14im
                ...
                @test maximum(abs.(k_gpu[:, 1:2])) > 1e-12
                @test maximum(abs.(k_gpu[:, 3:end])) < 1e-12
            end

            @testset "Rejected adaptive step preserves state" begin
                c = finish_config(make_radial(energy=6e-3))
                trial_dt = 0.1
