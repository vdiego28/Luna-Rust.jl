...
@testitem "Native-Rust CUDA free-space RealGrid thresholded ADK plasma (Plan 20)" tags=[:rust] begin
    ...
    if !isfile(libpath)
        require_cuda && error("CUDA Plan 20 tests require the Rust library")
        @test_skip "Rust library not found"
    else
        gas = :He; pres = 1.0; τ = 30e-15; λ0 = 800e-9
        w0 = 150e-6; energy = 1.6e-3; L = 0.02; R = 1.5e-3
        Nx = 10; Ny = 8
        ...
        normfun = NonlinearRHS.const_norm_free(
            grid, xygrid, PhysData.ref_index_fun(gas, pres))
        ...
        @testset "Eligibility and threshold boundaries" begin
            ...
            unthresholded = Ionisation.IonRateADK(
                ionpot; threshold=false, cycle_average=false)
            unthresholded_plasma = Nonlinear.PlasmaCumtrapz(
                grid.to, grid.to, unthresholded, ionpot)
            unthresholded_transform = NonlinearRHS.TransFree(
                grid, xygrid, transform.FT,
                (Nonlinear.Kerr_field(γ3), unthresholded_plasma), densityfun, normfun)
            @test !RK45._gpu_kernel_supports(unthresholded_transform, linop)
        end
        ...
        if !gpu_available
            ...
            @testset "Joint 3-D ADK stage and asymmetric layout" begin
                ...
                println("Free-space Plan 20 asymmetric ADK stage rel: ", rel_probe)
                @test rel_probe < 1e-8
                @test set_field(s_gpu._handle.ptr, Eω) == 0
                @test set_field(s_cpu._handle.ptr, Eω) == 0
            end

            @testset "Threshold boundaries and series isolation" begin
                s_cpu = make_cpu()
                s_boundary = make_gpu()
                ones_towin = ones(Float64, n_time_over)
                ones_m = ones(ComplexF64, n)
                @test set_free(s_cpu._handle.ptr, ones_towin, 0.0, ones_m) == 0
                @test set_free(s_boundary._handle.ptr, ones_towin, 0.0, ones_m) == 0
                @test set_adk(s_cpu._handle.ptr) == 0
                @test set_adk(s_boundary._handle.ptr) == 0

                # With M=1 and kerr=0, a temporal DC spectrum equal to the
                # 2-D FFT of the requested transverse field produces a
                # constant field in physical `(y,x)` space.  The spatial
                # inverse cancels its n_cols factor, leaving this temporal
                # scale for the requested boundary values.
                ...
                boundary_values = (0.0, 0.5 * ionrate.thr,
                                   0.0, 1000.0 * ionrate.thr)
                spatial_values = zeros(ComplexF64, Ny, Nx)
                for (j, value) in enumerate(boundary_values)
                    spatial_values[j] = value
                end
                spatial_spectrum = FFTW.fft(spatial_values)
                for q in 1:n_cols
                    boundary_fields[1 + (q - 1) * n_spec] =
                        scale_dc * spatial_spectrum[q]
                end
                @test set_field(s_cpu._handle.ptr, boundary_fields) == 0
                @test set_field(s_boundary._handle.ptr, boundary_fields) == 0
                k_cpu = reshape(getk(s_cpu), n_spec, n_cols)
                k_boundary = reshape(getk(s_boundary), n_spec, n_cols)
                @test norm(k_boundary - k_cpu) / max(norm(k_cpu), 1e-30) < 1e-10
                k_over = zeros(ComplexF64, n_spec_over, Ny, Nx)
                k_over[1:n_spec, :, :] = reshape(k_boundary, n_spec, Ny, Nx)
                k_phys = FFTW.ifft(FFTW.irfft(k_over, n_time_over, 1), (2, 3))
                k_phys_flat = reshape(k_phys, n_time_over, n_cols)
                col_max = vec(maximum(abs.(k_phys_flat), dims=1))
                @test count(>(1e-8), col_max[1:8]) == 2
                @test maximum(col_max[9:end]) < 1e-8
                @test maximum(col_max) > 1e-8
            end

            @testset "Failed ADK replacement is transactional" begin
                s_tx = make_gpu()
                ones_towin = ones(Float64, n_time_over)
                ones_m = ones(ComplexF64, n)
                @test set_free(s_tx._handle.ptr, ones_towin, 0.0, ones_m) == 0
                @test set_field(s_tx._handle.ptr, Eω) == 0
                @test set_adk(s_tx._handle.ptr) == 0
                k_before = getk(s_tx)
                bad_null = set_adk(s_tx._handle.ptr; ion_pointer=Ptr{Cvoid}(C_NULL))
                @test bad_null != 0
                @test norm(getk(s_tx) - k_before) / max(norm(k_before), 1e-30) < 1e-12
                bad_nan = set_adk(s_tx._handle.ptr; dt_value=NaN)
                @test bad_nan != 0
                @test norm(getk(s_tx) - k_before) / max(norm(k_before), 1e-30) < 1e-12
            end

            @testset "Non-vacuous strong field and trajectories" begin
                energy_strong = 1e-2
                ...
                Eω_strong, transform_strong, _ = with_logger(NullLogger()) do
                    Amalthea.setup(grid, xygrid, densityfun, normfun,
                                   responses, inputs_strong)
                end
                no_plasma = NonlinearRHS.TransFree(
                    grid, xygrid, transform_strong.FT,
                    (Nonlinear.Kerr_field(γ3),), densityfun, normfun)
                ...
                println("Free-space Plan 20 Julia ADK share: ", effect)
                @test effect > 100 * 1e-10

                s_gpu_strong = make_gpu(Eω_strong, transform_strong)
                solve(s_gpu_strong, L)
                rel_native = norm(s_gpu_strong.yn - s_on.yn) / norm(s_on.yn)
                println("Free-space Plan 20 strong native-vs-Julia rel: ", rel_native)
                ...
                println("Free-space Plan 20 fixed solve rel: ", rel_fixed)
                @test rel_fixed < 1e-6
                ...
                println("Free-space Plan 20 adaptive solve rel: ", rel_adapt)
                @test rel_adapt < 1e-6
