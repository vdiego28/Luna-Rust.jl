...
@testitem "Native-Rust CUDA modal RealGrid Kerr (Plan 14)" tags=[:rust] begin
    ...
    if !isfile(libpath)
        require_cuda && error("CUDA modal tests require the Rust library")
        ...
        flength = 0.05
        dt = 0.001
        grid = Grid.RealGrid(flength, λ0, (400e-9, 2000e-9), 0.5e-12)
        ...
        function make_case(; full=false, npol=1, two_modes=true, γ=γ3, energy=5e-6)
            ...
            elseif two_modes
                (Capillary.MarcatiliMode(radius, gas, pressure; kind=:HE, n=1, m=1),
                 Capillary.MarcatiliMode(radius, gas, pressure; kind=:HE, n=1, m=2))
            else
                (Capillary.MarcatiliMode(radius, gas, pressure; kind=:HE, n=1, m=1),)
            end
            ...
            Eω, transform, _ = with_logger(NullLogger()) do
                Amalthea.setup(grid, densityfun, responses, input, modes, components; full)
            end
        ...
        function modal_nodes(s, c, coords)
            ...
            out = zeros(Float64, length(coords) ÷ (c.full ? 2 : 1) * fdim)
            rc = ccall((:native_debug_modal_eval_nodes, libpath), Cint,
                       (Ptr{Cvoid}, Ptr{Float64}, Csize_t, Ptr{Float64}, Csize_t),
                       s._handle.ptr, coords,
                       Csize_t(length(coords) ÷ (c.full ? 2 : 1)), out,
                       Csize_t(length(out)))
            rc == 0 || error("native_debug_modal_eval_nodes failed with rc=$rc")
            out
        end

        function modal_stats(s)
            stats = zeros(Csize_t, 3)
            rc = ccall((:native_debug_modal_stats, libpath), Cint,
                       (Ptr{Cvoid}, Ptr{Csize_t}, Csize_t),
                       s._handle.ptr, stats, Csize_t(length(stats)))
            rc == 0 || error("native_debug_modal_stats failed with rc=$rc")
            stats
        end
        ...
        probe_error = try
            ...
            RK45._native_backend(probe) === :cuda ||
                error("explicit modal CUDA request selected $(RK45._native_backend(probe))")
            nothing
        catch err
            err
        end
        if probe_error !== nothing
            require_cuda && error("CUDA modal setup failed: $probe_error")
            ...
            @testset "Fixed-node device evaluator and direct stages" begin
                for c in cases
                    ...
                    rs = [0.11 * radius, 0.37 * radius, 0.73 * radius]
                    coords = c.full ? reduce(vcat, ([r, θ] for (r, θ) in
                                      zip(rs, (0.0, 0.7, 1.8)))) : rs
                    ...
                    println("CUDA modal $(c.full ? "full" : "radial") npol=$(c.npol) point rel: $point_rel")
                    @test point_rel < 5e-9
                    ...
                    println("CUDA modal $(c.full ? "full" : "radial") npol=$(c.npol) stage rel: $stage_rel")
                    @test stage_rel < 5e-8
                end
            end
            ...
            @testset "Fixed solve, transfer, and nonlinear non-vacuousness" begin
                ...
                s_cpu = make_stepper(c; gpu=false)
                s_gpu = make_stepper(c; gpu=true)
                t_cpu = @elapsed solve(s_cpu, flength)
                t_gpu = @elapsed solve(s_gpu, flength)
                solve_rel = norm(s_gpu.yn - s_cpu.yn) / norm(s_cpu.yn)
                println("CUDA modal fixed-solve rel: $solve_rel")
                @test solve_rel < 1e-6
                stats = modal_stats(s_gpu)
                println("CUDA modal device batches=$(stats[1]), H→D bytes=$(stats[2]), " *
                        "D→H bytes=$(stats[3]), CPU wall=$(t_cpu)s, GPU wall=$(t_gpu)s")
                @test stats[1] > 0
                @test stats[2] > 0
                @test stats[3] > 0
                he12_frac = sum(abs2, s_cpu.yn[:, 2]) / sum(abs2, s_cpu.yn)
                println("CUDA modal HE11→HE12 fraction: $he12_frac")
                @test he12_frac > 1e-6
                ...
                println("Julia modal Kerr on/off rel: $effect")
                @test effect > 1e-6
            end
